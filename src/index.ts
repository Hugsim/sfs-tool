...
const saveResponse = async (r: RiksdagenResponse): Promise<void> => {
    for (const d of r.dokumentlista.dokument) {
        if (await storage.has(d.dok_id)) {
            console.log(`Already have ${d.dok_id}!`);
            continue;
        }
...
var url: string =
    "https://data.riksdagen.se/dokumentlista/?sok=&doktyp=SFS&sort=datum&sortorder=desc&utformat=json";
while (true) {
